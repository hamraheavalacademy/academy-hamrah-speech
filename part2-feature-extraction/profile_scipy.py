"""Profile MFCC extraction with SciPy/NumPy.

Output is a torch.Tensor of shape [1, 13, Frames]. Configuration is identical
to the Librosa and PyTorch scripts. Audio loading/resampling is excluded from
the timed region.

Requirements:
    pip install scipy soundfile torch numpy psutil memray

Usage:
    python profile_scipy.py ../data/test1.wav 50
    memray run -o scipy_output.bin profile_scipy.py ../data/test1.wav 50
...
def extract_mfcc(signal):
    ...
    window = hamming(FRAME_LENGTH, sym=False).astype(np.float32)
    padding = N_FFT - FRAME_LENGTH
    window = np.pad(window, (padding // 2, padding - padding // 2))

    frames = np.lib.stride_tricks.sliding_window_view(signal, N_FFT)[::HOP_LENGTH]
    windowed = np.asarray(frames, dtype=np.float32) * window[None, :]
    spectrum = rfft(windowed, n=N_FFT, axis=1)
    power = (np.abs(spectrum) ** 2).astype(np.float32)
    mel_power = build_shared_mel_filterbank(TARGET_SR) @ power.T
