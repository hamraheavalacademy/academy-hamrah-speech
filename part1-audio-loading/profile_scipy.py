"""
Profile RTF, wall time, and CPU time for scipy.
...
Requirements:
    pip install scipy torch numpy

Usage:
    # Timing / RTF (clean, no instrumentation overhead):
    python profile_scipy.py audio.wav 50

    # Memory (run separately, so it doesn't skew the timing numbers above):
    pip install memray
    memray run -o scipy_output.bin profile_scipy.py audio.wav 50
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python profile_scipy.py path/to/audio.wav [n_runs]")
        sys.exit(1)
